...
def helper_script(bw, col_num):
    ...
    for row in bw:
        for item in row:
            if(item == '1'):
                list_of_indexes.append((x,y))
            x=x+1
        x=0
        y=y+1
    i = 0
    for coord in list_of_indexes:
        print("let d[" + str(i) + "] = Point.new(" + str(coord[0]) +"," + str(coord[1]) + ");")
        i=i+1

def bool_to_jack(bw, col_num, name):
    # print('\n'.join([''.join(['{:4}'.format(item) for item in row]) 
    #   for row in bw]))
    helper_script(bw, col_num)
